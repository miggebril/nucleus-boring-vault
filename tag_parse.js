...
function flattenType(input) {
  ...
  const arrayNotation = isArray
    ? input.type.substring(input.type.indexOf("["))
    : "";
...
function parseSourceFile(filePath) {
  try {
    ...
    while ((match = functionPattern.exec(content)) !== null) {
      ...
      // Extract @tag tags
      const tagMatches = [
        ...docComment.matchAll(/\/\/\s*@tag\s+(\w+):([^:]+)(?::(.*))?/g),
      ];
  ...
  for (const selector of selectors) {
    try {
      const result = await pb
        .collection("decoder_selectors")
        .getFirstListItem(`selector="${selector}"`);
      if (result) {
    ...
    for (const data of functionsData) {
      try {
        ...
        try {
          await pb
            .collection("decoder_selectors")
            .getFirstListItem(`selector="${data.selector}"`);
        ...
        console.error(
          red(`Error processing selector ${data.selector}: ${error.message}`),
        );
      }
    }
...
function findAllDecoderFiles(includeSubdirs = false) {
  ...
  try {
    ...
    const files = execSync(`find ${baseDir} ${depthOption} -name "*.sol"`, {
      encoding: "utf8",
    })
      .trim()
      .split("\n")
...
async function main() {
  ...
  if (processAllDecoders) {
    ...
    console.error(
      red(
        "Error: Please provide file paths as arguments or use --all-decoders flag",
      ),
    );
    console.error(
      red(
        "Usage: node tag_parse.js [file_paths...] [--post] [--all-decoders] [--include-subdirs]",
      ),
    );
    console.error(
      red("  --post: Post the data to PocketBase (otherwise dry run)"),
    );
    console.error(
      red(
        "  --all-decoders: Process all files in src/base/DecodersAndSanitizers",
      ),
    );
    console.error(
      red(
        "  --include-subdirs: Include files in subdirectories when using --all-decoders",
      ),
    );
    process.exit(1);
  }
